...
use url::Url;
use crate::backend::CustomProtocolBuilder;
...
use crate::{
    ...
    get_verso_path,
    utils::{to_tao_theme, to_cef_theme},
    webview::CefWebviewDispatcher,
    window::{CefWindowDispatcher, Window},
};
...
impl<T: UserEvent> RuntimeContext<T> {
    ...
    pub fn create_window<
        R: Runtime<
                T,
                WindowDispatcher = CefWindowDispatcher<T>,
                WebviewDispatcher = CefWebviewDispatcher<T>,
            >,
    ...
    >(
                ...
                for (scheme, handler) in &uri_scheme_protocols {
                    ...
                    if scheme == "ipc" {
                        if let Some(data) = request
                            .headers_mut()
                            .remove("Tauri-CefRuntime-Invoke-Body")
                        {
                            if let Ok(body) =
        ...
        let on_window_event_listeners: crate::window::WindowEventListeners =
            Arc::new(Mutex::new(HashMap::new()));

        // NOTE: Resized-event emission temporarily disabled while isolating a
        // ~8s self-crash (observers_.empty()).
...
/// A handle to the [`CefRuntime`] runtime.
#[derive(Debug, Clone)]
...
impl<T: UserEvent> RuntimeHandle<T> for CefRuntimeHandle<T> {
    ...
    fn set_device_event_filter(&self, filter: tauri_runtime::DeviceEventFilter) {}
        ...
        *self.context.prefered_theme.lock().unwrap() = theme;
        for window in self.context.windows.lock().unwrap().values() {
            if let Err(error) = window
                ...
                .lock()
                .unwrap()
                .set_theme(theme.map(to_cef_theme))
            {
...
impl<T: UserEvent> CefRuntime<T> {
    fn init(event_loop: EventLoop<Message<T>>) -> Self {
        // NOTE: CEF is initialized separately, at the very top of `main()` via
        // `tauri_runtime_cef::init_cef()`, because `execute_process`/`initialize`
        // must run before any threads are spawned. Do NOT init CEF here.
        ...
        self.event_loop
            .run_return(|event, event_loop, control_flow| {
                if *control_flow != ControlFlow::Exit {
                    // Wake periodically so CEF's message loop keeps getting
                    // pumped even when there are no tao events.
                    *control_flow = ControlFlow::WaitUntil(
                        std::time::Instant::now() + std::time::Duration::from_millis(8),
                    );
                }

                // Give CEF a slice of the main thread each iteration.
                crate::cef_app::pump();
                ...
                match event {
                    ...
                    TaoEvent::LoopDestroyed => {
                        // Flush the app's state (theseus cache/settings) first.
                        callback(RunEvent::Exit);
                        // Exit the process directly instead of calling
                        // `CefShutdown()`. After the event loop stops, CEF's
                        // window/browser teardown can't complete, so
                        // `CefShutdown()` aborts with `observers_.empty()`. Let
                        // the OS reclaim CEF's resources.
                        std::process::exit(0);
                    }
