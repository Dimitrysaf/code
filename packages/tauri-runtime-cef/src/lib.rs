//! # Tauri Runtime Cef
//!
//! Use Cef as the backend for Tauri
//!
//! ## Usage
//!
...
//! ### Devtools
//!
//! Since Cef doesn't have a devtools built-in, you'll need to use the one from the Firefox, first put in this in your code
//!
//! ```rust
...
pub use runtime::{EventProxy, RuntimeContext, CefRuntime, CefRuntimeHandle};
pub use webview::CefWebviewDispatcher;
pub use window::{CefWindowBuilder, CefWindowDispatcher};

/// Initializes CEF for this process. Must be called once at the very top of
/// `main()`, before any threads are spawned (a CEF requirement on Linux).
///
/// In a CEF helper subprocess (renderer/GPU/utility) this runs the subprocess to
/// completion and exits the process; in the browser process it initializes CEF
/// and returns.
pub fn init_cef() {
    cef_app::init();
}
...
/// Sets the Cef executable path to ues for the webviews,
...
pub fn set_verso_path(path: impl Into<PathBuf>) {
    VERSO_PATH
        .set(path.into())
        .expect("Cef path is already set, you can't set it multiple times");
}

fn get_verso_path() -> &'static Path {
    VERSO_PATH.get_or_init(|| {
        relative_command_path("versoview").expect(
            "Cef path not set! You need to call set_verso_path before creating any webviews!",
        )
    })
}
...
/// Sets the Cef resources directory to ues for the webviews,
...
///     tauri_runtime_verso::set_verso_path("../verso/target/debug/versoview");
///     tauri_runtime_verso::set_verso_resource_directory("../verso/resources");
///     tauri::Builder::<tauri_runtime_verso::CefRuntime>::new()
...
/// Sets the Cef devtools port to ues for the webviews, 0 for random port,
/// note this only affects webviews created after you set this
///
/// Since Cef doesn't have devtools built-in,
...
/// Creates a new [`tauri::Builder`] using the [`CefRuntime`]
///
/// ### Example:
///
/// ```no_run
...
/// }
/// ```
pub fn builder() -> tauri::Builder<CefRuntime> {
