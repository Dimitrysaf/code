...
use crate::backend::{CefBuilder, CefviewController};
#[cfg(windows)]
...
impl Default for CefWindowBuilder {
    fn default() -> Self {
        let mut verso_builder = CefBuilder::new();
...
impl WindowBuilder for CefWindowBuilder {
    ...
    fn with_config(config: &WindowConfig) -> Self {
        ...
        if let Some(theme) = config.theme {
            verso_builder = verso_builder.theme(to_cef_theme(theme));
        }
    ...
    fn focusable(mut self, focusable: bool) -> Self {
        self.verso_builder = self.verso_builder.focused(focusable);
        self
    }
    ...
    fn always_on_bottom(mut self, always_on_bottom: bool) -> Self {
        self.verso_builder = self.verso_builder.window_level(if always_on_bottom {
            crate::backend::WindowLevel::AlwaysOnTop
        } else {
            crate::backend::WindowLevel::Normal
        });
        self
    }
    ...
    fn icon(mut self, icon: Icon<'_>) -> Result<Self> {
        self.verso_builder = self.verso_builder.icon(crate::backend::Icon {
    ...
    fn theme(mut self, theme: Option<Theme>) -> Self {
        if let Some(theme) = theme {
            self.verso_builder = self.verso_builder.theme(to_cef_theme(theme));
...
/// The Tauri [`WindowDispatch`] for [`CefRuntime`].
#[derive(Clone)]
...
impl<T: UserEvent> Debug for CefWindowDispatcher<T> {
    ...
    fn set_always_on_bottom(&self, always_on_bottom: bool) -> Result<()> {
        self.webview
            ...
            .set_window_level(if always_on_bottom {
                crate::backend::WindowLevel::AlwaysOnBottom
            } else {
                crate::backend::WindowLevel::Normal
            })
            .map_err(|_| Error::FailedToSendMessage)?;
